...
超过 12 个视觉单位的子句才调用模型；视觉长度计入汉字、数字表达式和 Latin 单词。长子句按最多 256 个 Unicode token 的固定窗口各推理一次，相邻窗口共享一个 token，保证每个候选边界恰好评分一次。后续递归保留这些原始 logits，只在当前子句内部的全部 gap 上重算 softmax，默认只允许概率严格大于 **75%** 的位置切分。词语、数量和标点附着保护继续生效；符合条件的位置直接按模型原始 logit 排序，同分时取当前片段内靠前的位置。过滤受保护的位置不会再次归一化概率。
