...
需要切分的子句先生成一份固定的模型评分，后续拆分只在当前片段范围内选择评分最高的允许位置，不再次推理。词语与数量保护仍排除其范围内的候选位置，同分时保留第一个候选，停止阈值仍是 8 视觉单位。长子句按最多 256 个汉字的固定窗口各推理一次，相邻窗口共享一个汉字，保证每个候选边界恰好评分一次；窗口边缘不强制切分。评分只保留到本次子句处理结束。这些计算细节不拆分视口请求，也不影响 toggle 的锁。
