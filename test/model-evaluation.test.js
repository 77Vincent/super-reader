...
test("evaluation uses regenerated list fragments and records the Unicode context standard", async (t) => {
  ...
  const records = buildAdjacentSamples({
    id: "news:gold", domain: "news", text: "引句。苹果、香蕉，准备做果汁。",
  });
...
test("default evaluation selects the corrected holdout even when a legacy generic split exists", (t) => {
  ...
  const dataDirectory = join(directory, "training/data/processed/chinese-line-web-100m-16conv-v7-20260917-eval");
  ...
  for (const name of ["training/evaluate_model.mjs", "training/text_policy.mjs", "training/text-policy.json", "training/unicode-symbols.json", "src/backend/inference.js",
